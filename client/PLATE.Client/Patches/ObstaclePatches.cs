...
namespace PLATE.Client.Patches
{
    ...
    /// arguments of Shot.Create by ShotLifecyclePatches, because the engine builds the
    ...
    internal static class ObstaclePatches
    {
        ...
        private static Shot _resolvedShot;
        ...
        private static Shot _verdictShot;
        ...
        private static Shot _freeExitShot;
        ...
        /// </summary>
        internal static bool TryAnchorDistanceMm(Shot shot, BallisticCollider collider,
        ...
        /// </summary>
        private static double ColliderExtentMm(Shot shot)
        {
            try
            {
                var hit = shot._hit.collider;
        ...
        /// vanilla's own.
        /// </summary>
        internal static bool FreeExitThisHit(Shot shot, BallisticCollider collider)
        {
        ...
        /// the prefix on Shot.Create — see ShotLifecyclePatches for why it has to be
        ...
        /// </summary>
        internal static bool TryChildLaunch(Shot parent, Vector3 vanillaDirection,
            out float speedMs, out Vector3 direction, out bool rebuildDirection)
        {
            ...
            try
            {
                ...
                var launch = ObstacleModel.Launch(pierced, _exit, bounced, _bounceRetention,
                    parent._currentVelocity.magnitude);
        ...
        /// </summary>
        private static Vector3 DeviatedDirection(Shot parent, float spread)
        {
            var dir = parent._currentVelocity.sqrMagnitude > 1e-6f
                ? parent._currentVelocity.normalized
                : parent.Direction;
        ...
        /// </summary>
        private static Vector3 FlattenedBounce(Vector3 vanillaDirection, Shot parent)
        {
            var dir = vanillaDirection.sqrMagnitude > 1e-6f
                ? vanillaDirection.normalized
                : parent._currentVelocity.sqrMagnitude > 1e-6f
                    ? parent._currentVelocity.normalized
                    : parent.Direction;
        ...
        private static bool HasVerdict(Shot shot)
        {
        ...
        /// </summary>
        internal static bool DeformedThisHit(Shot shot)
        {
        ...
        /// </summary>
        internal static bool TryThicknessUsedMm(Shot shot, BallisticCollider collider,
        ...
        /// </summary>
        private static bool TryResolve(BallisticCollider collider, Shot shot,
            ...
            out ObstacleModel.Outcome outcome)
        {
            ...
            var mixed = EFT.Utilities.FastRndom.RandomizeInt(shot.RandomSeed);
            ...
            outcome = ObstacleModel.Resolve(projectile, barrier,
                ObstacleReference.TuningOf(book), shot._hitCosDirectionToNormal, draw,
                mixed);
        ...
        /// </summary>
        private static bool IsPenetratedPrefix(BallisticCollider __instance, Shot shot,
            ref bool __result)
        {
            ...
            try
            {
                ...
                var cos = shot._hitCosDirectionToNormal;
        ...
        /// </summary>
        private static bool DeflectsPrefix(BallisticCollider __instance, Shot shot,
            Vector3 shotNormal, ref bool __result)
        {
            ...
            try
            {
                ...
                var alpha = ObstacleModel.GrazeAngleDeg(shot._hitCosDirectionToNormal);
        ...
        /// </summary>
        private static bool FragmentationPrefix(Shot __instance, ref bool __result)
        {
        ...
        /// </summary>
        private static void DeviatedChildPostfix(Shot __instance)
        {
        ...
        /// </summary>
        internal static bool TryMeasureThicknessMm(Shot shot, out double thicknessMm)
        {
            thicknessMm = 0;
            var begin = PerfTrace.Begin();
            try
            {
                var collider = shot._hit.collider;
                if (collider == null)
                {
                    return false;
                }

                var dir = shot._currentVelocity;
        ...
        private static ObstacleModel.Projectile Read(Shot shot)
        {
            ...
            return new ObstacleModel.Projectile
            {
                MassG = massG,
                DiaMm = diaMm,
                V = shot._currentVelocity.magnitude,
                X = x,
        ...
        /// </summary>
        private static float EffectiveYaw(Shot shot)
        {
        ...
        /// </summary>
        private static void NoteUnmapped(BallisticCollider collider, Shot shot)
        {
        ...
        private static bool WorthLogging(Shot shot)
        {
        ...
        private static void Log(Shot shot, BallisticCollider collider,
