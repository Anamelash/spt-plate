...
namespace PLATE.Client.Patches
{
    ...
    internal static class OverlayPatches
    {
        ...
        /// slot.
        /// </summary>
        private static void ObstacleHitPostfix(Shot __instance)
        {
            ...
            try
            {
                ...
                if (wantSurvey)
                {
                    ...
                    ObstacleSurvey.Note(
                        ...
                        chord, __instance._hitCosDirectionToNormal, got,
                        Time.time, freeExit);
                }
                ...
                var stopped = state == nameof(Shot.EBulletState.StopHit);
                var ricochet = state == nameof(Shot.EBulletState.RicochetHit);

                var vIn = __instance._currentVelocity.magnitude;
                var dirIn = __instance._currentVelocity.sqrMagnitude > 1e-8f
                ...
                if (child != null)
                {
                    vOut = child._currentVelocity.magnitude;
                    if (child._currentVelocity.sqrMagnitude > 1e-8f)
                    {
                        devDeg = Vector3.Angle(dirIn, child._currentVelocity.normalized);
                    }
                }
