...
namespace PLATE.Client.Patches
{
    ...
    internal static class BallisticsPatches
    {
        ...
        /// </summary>
        internal static string AmmoLabel(Shot shot)
        {
        ...
        /// </summary>
        internal static bool TryChildLaunch(Shot parent, out float speedMs, out float massG,
            out float diaMm)
        {
            speedMs = 0f;
            massG = 0f;
            ...
            try
            {
                ...
                var fragmenting = parent.BulletState == Shot.EBulletState.FragmentationHit;
                if (!fragmenting && (parent.BulletState != Shot.EBulletState.DeviationHit ||
                                     !parent.IsForwardHit))
                {
                ...
                var v = parent._currentVelocity.magnitude;
                var x = EffectiveX(parent);
                var spread = ShotSpread.For(parent, parentDia, wound);
                var chord = ChordMm(bpc, parent._hit.point, parent._currentVelocity, parentDia);
        ...
        /// </summary>
        private static int FragmentCount(Shot shot)
        {
            return EFT.Utilities.FastRndom.Int(shot.RandomSeed, shot.MinFragmentsCount,
