...
namespace PLATE.Client.Patches
{
    ...
    /// happen nowhere else.
    ///
    /// <b>The trajectory table is built at birth.</b> `EftBulletClass.Create` forms the whole
    ...
    /// raid, and `method_0` clears the engine's own fields and knows nothing of
    ...
    internal static class ShotLifecyclePatches
    {
        public static void Apply(Harmony harmony)
        {
            ...
            if (target == null)
            {
                ...
                PatchStats.MarkFailed(null, Label(nameof(ForgetPooledStatePostfix)),
                    "target not resolved");
                Plugin.Log.LogError("[PLATE] EftBulletClass lifecycle: EftBulletClass.Create not resolved, skipped");
                return;
            }
            ...
            catch (Exception ex)
            {
                ...
                Plugin.Log.LogError($"[PLATE] EftBulletClass lifecycle: failed to patch Create: {ex.Message}");
            }
        }
        ...
        /// </summary>
        private static void ForgetPooledStatePostfix(EftBulletClass __result)
        {
        ...
        private static void LogError(string where, Exception ex)
        {
            ...
            Plugin.Log.LogError($"[PLATE] EftBulletClass lifecycle {where}: {ex}");
        }
    }
}
