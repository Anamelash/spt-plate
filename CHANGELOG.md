...
The obstacle model reaches SPT 4.0.13. Cover stops being a coin flip on this
line too: a wall, a door or a sheet of tin is a material and a thickness, and a
bullet that crosses one arrives on the far side with what the crossing left it.
Everything 1.4.0 shipped is here, and the physics is the same physics —
`docs/MODEL.md` is the document both lines are built from.

- **Cover is a barrier now.** The section above spells it out in full: penetration
  decided by the material rather than by a designer's number and a dice roll,
  energy and mass paid for the hole, yaw picked up on the way through so the next
  wall meets the bullet sideways, physical ricochet with its own angle window per
  surface class, and packed cargo resolved as random inclusions instead of one
  averaged block. Shooting through a door is a decision with a cost, not a free
  reroll.
- **The projectile that comes out of a wall is a different projectile.** Its exit
  state — speed, direction, mass, calibre, deformation and yaw — is written where
  the game builds a shot's trajectory table, so it survives the first tick. On the
  old build a round through a door arrived on the far side at nearly muzzle
  velocity; it does not any more.
- **Hit markers you can walk up to.** Optional 3D markers show where your shots
  landed, on bodies and on cover, with the material and the thickness the model
  actually used. Off by default, along with the obstacle journal that logs what the
  model decided next to what the game did.
- **Bullet length comes from measurement where the book has it.** Five cartridges
  carry a published length now instead of one inferred from mass, which stops a
  light steel-cored round being read as a sphere by the yaw model.
- **Ghost mode and a player speed multiplier**, for walking a map and surveying
  cover without an argument. Debug tools, off by default.

- **SPT 4.0.13 only**, as always on this line. The server half is built against
  `SPTarkov.Server.Core` 4.0.13 and the client against the 4.0 game assemblies;
  no 1.x release loads here and this one does not load on 4.1. Building from
  source needs the .NET 9 SDK.
- The client writes `obstacle-reference.jsonc` beside the plugin on first start.
  It is the obstacle module's only configuration — the constants are not
  duplicated in the F12 file — and your edits survive a restart.
