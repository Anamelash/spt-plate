...
**Current release:** P.L.A.T.E. 0.12.0
...
[latest 0.x release](https://github.com/Anamelash/spt-plate/releases/tag/v0.12.0)
with SPT 4.0.13. Version 0.12.0 is the backport of 1.4.0 and has the same
features, obstacle model included. On SPT 4.1.x install a 1.x release instead.
...
The leading version number identifies the SPT server line, not the amount of
mod behind it. Version 0.12.0 is the current backport for SPT 4.0.13 and has
the same features as 1.4.0, obstacle model included. Neither branch loads on
the other SPT server line.
...
The server targets net9, so building requires the .NET 9 SDK. Set the game
