...
namespace PLATE.Tests
{
    ...
    /// assembly.
    ///
    /// Neither needs a running game: a Shot constructs headless (its constructor only
    ...
    public class ShotLifecycleTests : IClassFixture<GameFixture>
    {
        ...
        private static Shot Node(BallisticCollider hit, Vector3 at, Shot parent)
        {
            var shot = new Shot
            {
                Parent = parent,
        ...
        /// nobody had checked. `PrepareToPool` nulls the engine's own references and does
        ...
        public void Pooling_a_shot_does_not_change_its_identity()
        {
            ...
            var shot = new Shot { RandomSeed = 12345 };
            shot.PrepareToPool();
