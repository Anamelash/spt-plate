...
namespace PLATE.Tests
{
    ...
    public class WallJournalTests
    {
        ...
        /// </summary>
        [Theory]
        [InlineData(Shot.EBulletState.DeviationHit, false, "intact")]
        [InlineData(Shot.EBulletState.DeviationHit, true, "deformed")]
        [InlineData(Shot.EBulletState.FragmentationHit, false, "destroyed")]
        [InlineData(Shot.EBulletState.StopHit, false, "stopped")]
        [InlineData(Shot.EBulletState.RicochetHit, false, "ricochet")]
        public void Every_bullet_state_maps_to_an_effect(Shot.EBulletState state,
        ...
        public void Deformed_only_overrides_a_pass_through()
        {
            Assert.Equal("stopped",
                WallJournal.EffectOf(nameof(Shot.EBulletState.StopHit), deformed: true));
            Assert.Equal("ricochet",
                WallJournal.EffectOf(nameof(Shot.EBulletState.RicochetHit), deformed: true));
        }
        ...
        {
            Assert.Equal("flying",
                WallJournal.EffectOf(nameof(Shot.EBulletState.Flying), deformed: false));
        }
